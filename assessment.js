...
function removeItem(array, string) {
	var newArray = array
	var takeOutThis = string
	for (var i = newArray.length - 1; i >= 0; i--)
		if (array[i] === takeOutThis) {
			array.splice(i, 1);
		}
	return array
}
...
//It looks to see if the person’s name is Jeremy and then changes the person object to have a property called lovesCode
...
function outside(temp, hum, clo) {
	//console.log(temp, hum, clo)
	if (temp > 80 && hum > 40) {
		return "I'm all sweat"
	}
	//console.log(temp, hum, clo)
	else if (temp < 40 && clo > 60) {
		return 'I have icecicles'
	}
	
	if (temp > 80 && hum < 40 && clo < 20) {
		return "I'm literally in the desert"
	}
	
	if (temp > 80 || hum > 50 || clo > 50) {
